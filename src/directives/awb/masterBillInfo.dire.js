...
module.exports = ['$document', '$rootScope', 'restAPI', 'Notification',
  function ($document, $rootScope, restAPI, Notification) {
    return {
      restrict: 'EA',
      template: require('./masterBillInfo.html'),
      replace: true,
      scope: {
        billno: '='
      },
      link: function (scope, element, attrs, ctrls, transcludeFn) {
        ...
        function getImg() {
          $rootScope.loading = true;
          restAPI.barCode.barCode.save({}, scope.billno)
            .$promise.then(function (resp) {
              $rootScope.loading = false;
              if (resp.ok) {
        ...
        function setImg(params) {
          ...
          var element = angular.element(document.getElementsByClassName("footer"));
          element.css({
            display: 'none'
          });
        } 
