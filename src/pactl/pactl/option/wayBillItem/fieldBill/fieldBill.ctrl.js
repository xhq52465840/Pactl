...
module.exports = ['$scope', 'Page', 'restAPI', '$state', 'Notification', '$rootScope', '$modal',
  function ($scope, Page, restAPI, $state, Notification, $rootScope, $modal) {
    ...
    function getFieldData() {
      $rootScope.loading = true;
      restAPI.fieldBill.fieldList.save({}, {})
        .$promise.then(function (resp) {
    ...
    function add() {
      ...
      addFieldDialog.result.then(function (data) {
        ...
        restAPI.fieldBill.addField.save({}, obj)
          .$promise.then(function (resp) {
            if (resp.ok) {
              search();
    ...
    function edit(param) {
      var editFieldDialog = $modal.open({
        ...
        resolve: {
          items: function () {
            return {
              title: '编辑运单关键字段: ' + param.fieldMaintain.field,
              field: vm.field,
              obj: {
      ...
      var delDialog = $modal.open({
        ...
        resolve: {
          items: function () {
            return {
              title: '删除：' + name,
              content: '你将要删除运单关键字段' + name + '。此操作不能恢复。'
            };
          }
        }
      });
      delDialog.result.then(function () {
        restAPI.fieldBill.delField.save({}, {
            id: id
          })
          .$promise.then(function (resp) {
            if (resp.ok) {
