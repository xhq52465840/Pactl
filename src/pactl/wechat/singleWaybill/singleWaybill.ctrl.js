...
module.exports = ['$scope', '$http', '$rootScope', '$filter', '$cookieStore', '$timeout',
	function($scope, $http, $rootScope, $filter, $cookieStore, $timeout) {
		'use strict';
		var vm = $scope;
		var token = '';
		var stateParams = {};
		...
		vm.onlyEn = function() {
				vm.single.waybillNo = vm.single.waybillNo.replace(/[^0-9]/g, '');
			}
			/**
			 * 根据运单号查询
			 */
		vm.search = function(param) {
			$http({
					url: '/api/wechat/wechat/waybill/agent/waybillInfo/' + token,
					method: "POST",
					data: {
						waybillNo: param
					}
