...
var printBattery_fn = ['$scope', 'restAPI', 'Auth', '$window', '$state', '$stateParams', '$timeout', '$resource', 'printHtml',
  function ($scope, restAPI, Auth, $window, $state, $stateParams, $timeout, $resource, printHtml) {
    ...
    function check() {
      var lang = $stateParams.lang || 'CN';
      var url = '../../../i18n/printBatteryDeclaraction/' + lang + '.json';
      $resource(url).get().$promise.then(function (resp) {
    	  console.log(resp)
        vm.showObj = resp;
        search();
      });
...
module.exports = angular.module('app.agentPrejudice.print', []).controller('printBatteryDeclaractionCtrl', printBattery_fn);