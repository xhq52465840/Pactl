 ...
 */
function getLinkFunction($http, util, type, $window) {
    return function (scope, element, attrs) {
        scope.config = scope.config || {};
        var ndWrapper = element.find('div')[0], ndParent = element.parent()[0],  width, height, chart;
        var chartEvent = {};
        function getSizes(config) {
            var parentWidth = ndParent.clientWidth, parentHeight = ndParent.clientHeight;
        ...
        function getOptions(data, config, type) {
            ...
            if(angular.isArray(config.xAxis)){
                options.xAxis = config.xAxis;
            }
            if(angular.isArray(config.yAxis)){
...
for (var i = 0, n = types.length; i < n; i++) {
    (function (type) {
        app.directive(type + 'Chart', ['$http', 'util', '$window', function ($http, util, $window) {
                    return {
                        restrict: 'EA',
                        ...
                        link: getLinkFunction($http, util, type, $window)
                    };
                }]);
    }(types[i]));
}
...
angular.module('angular-echarts.util', []).factory('util', function () {
    ...
    function getSeries(data, config, type) {
        ...
        angular.forEach(data, function (serie) {
            ...
            if (type === 'bar' && angular.isArray(config.xAxis)) {
