    ...
    this.$get = ["$timeout", "$http", "$compile", "$templateCache", "$rootScope", "$injector", "$sce", "$q", "$window", function ($timeout, $http, $compile, $templateCache, $rootScope, $injector, $sce, $q, $window) {
        ...
        var notify = function (args, t) {
            ...
            function processNotificationTemplate(template) {
                ...
                var reposite = function () {
                    ...
                    for (var i = messageElements.length - 1; i >= 0; i--) {
                        ...
                        if (element._positionX == 'center') {
                            element.css('left', parseInt(getWidth() / 2 - elWidth / 2) + 'px');
                        } else {
                ...
                function doTing() {
                    doDelay = $timeout(function () {
                        var ienum = parseInt((/msie (\d+)/.exec(navigator.userAgent.toLowerCase()) || [])[1], 10);
                        if (ienum === 8 || ienum === 9) {
                            scope.kill(true);
                        } else {
                ...
                if (args.positionX == 'center') {
                    ...
                    // if()
                    templateElement.css('left', parseInt(getWidth() / 2 - elWidth / 2) + 'px');
                }
                ...
                function setCssTransitions(value) {
                    angular.forEach(['-webkit-transition', '-o-transition', 'transition'], function (prefix) {
...
angular.module("ui-notification").run(["$templateCache", function ($templateCache) {
    $templateCache.put("angular-ui-notification.html", "<div ng-mouseover=\"mouseover()\" ng-mouseleave=\"mouseleave()\" class=\"ui-notification\"><h3 ng-show=\"title\" ng-bind-html=\"title\"></h3><div class=\"message\" ng-bind-html=\"message\"></div></div>");
}]);