...
module.exports = function (i) {
  ...
  function mouseWheelHandler(e) {
    ...
    // update slider
    i.sumDeltaY += (e.deltaY || e.wheelDelta);
    var newTop = 0;
  ...
  if(i.content.element.addEventListener){
    event.bind(i.content.element, 'wheel', mouseWheelHandler);
  }else{
    event.bind(i.content.element, 'mousewheel', mouseWheelHandler);
  }
};
...
function createSliderYElement() {
  ...
  div.setAttribute('class','nice-bar-slider-y');
  return div;
}

function createRailYElement() {
  var div = document.createElement('div');
  div.setAttribute('class','nice-bar-rail-y');
  return div;
}
...
function getCss(element, styleName) {
  var styleValue = '';
    if (typeof window.getComputedStyle !== 'undefined') {
        styleValue = window.getComputedStyle(element, null).getPropertyValue(styleName);
    } else {
        styleValue = element.currentStyle[styleName];
    }
  
  // var styleValue = window.getComputedStyle
...
var event = {
  bind: function (element, name, listener) {
    if(element.addEventListener){
      ...
      element.attachEvent('on'+name, listener, false);
    }
  },
  ...
  unbind: function (element, name, listener) {
    ...
    if (element.removeEventListener) {
      ...
      element.detachEventListener('on'+name, listener, false);
    }
  },
